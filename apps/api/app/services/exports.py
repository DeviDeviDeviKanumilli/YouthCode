...
class ExportService:
    ...
    async def list_exports(
        ...
        if requester_id is None:
            return await self.repository.list_for_requester(owner_id)
        requester = await self._require_export_history_access(requester_id)
        if requester.role == UserRole.admin:
            return await self.repository.list_for_requester(owner_id)
        if owner_id is not None and owner_id != requester_id:
            raise AppError(
                code="export_history_forbidden",
                message="Non-admin users can only list their own exports.",
                status_code=status.HTTP_403_FORBIDDEN,
            )
    ...
    async def get_export(
        ...
        if requester_id is not None:
            requester = await self._require_export_history_access(requester_id)
            if requester.role != UserRole.admin and export.requester_id != requester_id:
                raise AppError(
                    code="export_history_forbidden",
                    message="Non-admin users can only inspect their own exports.",
                    status_code=status.HTTP_403_FORBIDDEN,
                )
        return export
    ...
    async def _require_export_history_access(self, requester_id: uuid.UUID) -> Any:
        ...
        if requester.role not in {UserRole.researcher, UserRole.reviewer, UserRole.admin}:
            raise AppError(
                code="export_history_forbidden",
                message="Only research, reviewer, or admin users can view export history.",
                status_code=status.HTTP_403_FORBIDDEN,
            )
        return requester
